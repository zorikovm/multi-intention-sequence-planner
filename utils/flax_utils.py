...
def restore_agent(agent, restore_path, restore_epoch):
    ...
    elif restore_epoch is None:
        # Released evaluation checkpoints use the unnumbered ``params.pkl``
        # name, whereas checkpoints emitted during training are numbered.
        restore_path = os.path.join(restore_dir, 'params.pkl')
    else:
        restore_path = os.path.join(restore_dir, f'params_{restore_epoch}.pkl')
